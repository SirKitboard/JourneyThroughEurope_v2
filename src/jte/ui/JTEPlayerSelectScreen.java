...
public class JTEPlayerSelectScreen extends BorderPane{
	...
	public JTEPlayerSelectScreen() {
		...
		final ComboBox<Integer> comboBox2 = new ComboBox<Integer>();
		comboBox2.getItems().addAll(2, 3, 4, 5, 6, 7, 8);
		comboBox.setValue(2);
		comboBox2.setValue(2);
		Label noPlayers = new Label("Select Number of Players : ");
		Label noCards = new Label("Select Number of Cards To be Dealt: ");
		noPlayers.setMinWidth(150);
		noPlayers.setMinWidth(200);
		playerSelectNorthBar.getChildren().addAll(noPlayers, comboBox,noCards, comboBox2, go);
		comboBox.valueProperty().addListener(e -> numberOfPlayers(comboBox.getValue()));
		go.setOnAction(e -> ui.startGame(comboBox.getValue(),comboBox2.getValue()));
